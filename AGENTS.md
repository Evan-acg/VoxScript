...
- 分层：`src/command`（编排：`cli.py` 入口 + `pipeline.py` 管线）→ `src/handler`（asr/audio/subtitle）→ `src/entity`（pydantic 模型）→ `src/parser`（字幕解析策略）→ `src/ui`（rich Live 面板）；`src/core` 为 events/config/preflight。
